...
def test_a_block_reads_its_fields_as_the_differences_of_a_running_sum(depth: BitDepth) -> None:
    ...
    frames = decompress(stated(differences, depth=depth), frames=len(differences), depth=depth, doubled=False)
...
def test_the_later_compressor_sums_a_second_time(depth: BitDepth) -> None:
    differences = [4, 1, -2, 3]
    frames = decompress(stated(differences, depth=depth), frames=len(differences), depth=depth, doubled=True)
...
def test_a_field_announcing_a_narrower_width_reads_the_rest_at_it(depth: BitDepth) -> None:
    ...
    frames = decompress(writer.block(), frames=3, depth=depth, doubled=False)
...
def test_a_field_in_the_middle_range_announces_a_width_too() -> None:
    ...
    frames = decompress(writer.block(), frames=2, depth=BitDepth.EIGHT, doubled=False)
...
def test_a_waveform_longer_than_one_block_reads_across_them() -> None:
    ...
    frames = decompress(data, frames=len(differences), depth=depth, doubled=False)
...
def test_a_waveform_of_no_frames_reads_as_nothing() -> None:
    assert decompress(b"", frames=0, depth=BitDepth.EIGHT, doubled=False).size == 0
...
def test_a_block_announcing_a_width_its_depth_leaves_no_room_for_is_refused() -> None:
    writer = BitWriter()
    writer.put(WIDE_MARKER[BitDepth.EIGHT] | 0xFF, 9)
    writer.put(0, 8)
    with pytest.raises(ValueError, match="announces width"):
        decompress(writer.block(), frames=1, depth=BitDepth.EIGHT, doubled=False)
