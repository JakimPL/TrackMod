...
from trackmod.trackers.mod.timing import exact_timings, nearest_timing, row_frames
...
def test_the_clock_a_module_starts_on_lasts_the_length_the_lineage_reads() -> None:
    ...
    assert row_frames(DEFAULT_SPEED, DEFAULT_TEMPO, frame_rate=FRAME_RATE) == round(FRAME_RATE * 6 * 0.02)
...
def test_the_lattice_reaches_no_further_than_the_one_command_that_states_it() -> None:
    ...
    timings = exact_timings(frame_rate=FRAME_RATE, speed=DEFAULT_SPEED)
    assert timings
...
def test_a_target_row_length_lands_on_the_nearest_clock_the_format_reaches() -> None:
    target = row_frames(DEFAULT_SPEED, 140, frame_rate=FRAME_RATE)
    assert nearest_timing(target, frame_rate=FRAME_RATE, speed=DEFAULT_SPEED).tempo == 140


def test_a_speed_the_command_reaches_shortens_the_row_it_states() -> None:
    slow = row_frames(MAX_EFFECT_SPEED, DEFAULT_TEMPO, frame_rate=FRAME_RATE)
    quick = row_frames(1, DEFAULT_TEMPO, frame_rate=FRAME_RATE)
