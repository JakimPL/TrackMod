...
def check_song(checklist: Checklist, song: Song) -> None:
    ...
    The width is pinned as well as the clock, because the header states neither: every module of this
    format plays four channels at the one speed and tempo its machine started on, so a song asking for
    anything else is told so and what it asked for stays visible.
    """
