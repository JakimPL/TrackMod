...
from trackmod.spec.application import APPLICATION_MARK, APPLICATION_NAME
from trackmod.trackers.s3m.spec.identity import TRACKER_BITS, VERSION_MASK
...
class Tracker(StrEnum):
    ...
    Scream Tracker 3 settled the field and the trackers after it each took a number of their own to sit
    above the version they state, so the number is what says which program laid a file out.
    """

    SCREAM_TRACKER = "scream_tracker"
    IMAGO_ORPHEUS = "imago_orpheus"
    IMPULSE_TRACKER = "impulse_tracker"
    SCHISM_TRACKER = "schism_tracker"
    OPEN_MPT = "openmpt"
    SOUND_CLUB = "sound_club"
    TRACKMOD = "trackmod"
...
LABELS: Final[Mapping[Tracker, str]] = {
    Tracker.SCREAM_TRACKER: "Scream Tracker 3",
    Tracker.IMAGO_ORPHEUS: "Imago Orpheus",
    Tracker.IMPULSE_TRACKER: "Impulse Tracker",
    Tracker.SCHISM_TRACKER: "Schism Tracker",
    Tracker.OPEN_MPT: "OpenMPT",
    Tracker.SOUND_CLUB: "Sound Club",
    Tracker.TRACKMOD: APPLICATION_NAME,
}

TRACKER_NUMBERS: Final[Mapping[int, Tracker]] = {
    0x1: Tracker.SCREAM_TRACKER,
    0x2: Tracker.IMAGO_ORPHEUS,
    0x3: Tracker.IMPULSE_TRACKER,
    0x4: Tracker.SCHISM_TRACKER,
    0x5: Tracker.OPEN_MPT,
}

TRACKER_SIGNATURES: Final[Mapping[bytes, Tracker]] = {
    b"SCLUB2.0": Tracker.SOUND_CLUB,
    APPLICATION_MARK: Tracker.TRACKMOD,
}
...
def wrote(created_with: int) -> Tracker | None:
    ...
    A field carrying a number no program here claims leaves the writer unnamed.
    """
...
def signed(signature: bytes) -> Tracker | None:
    ...
    The eight bytes this format reserves are where a tracker writes a mark of its own, so a file
    carrying one names its writer more closely than the number above its version does.
    """
...
def stated_provenance(*, created_with: int, signature: bytes) -> Provenance:
    ...
    it is there. Behind it stands the number above the version, which every file states.
    """
