...
def row_frames(speed: int, tempo: int, *, frame_rate: int) -> int:
    """The frames one Scream Tracker 3 row spans, bound to this format's speed and tempo ranges."""
...
def exact_timings(*, frame_rate: int, speed: int) -> list[Timing]:
    """Every Scream Tracker 3 tempo whose row is a whole number of frames at one speed."""
...
def nearest_timing(target_frames: int, *, frame_rate: int, speed: int) -> Timing:
    """The Scream Tracker 3 timing whose row length is closest to a target."""
