...
def module_bytes(song: Song, *, storage: Storage) -> SizeReport:
    """How many bytes a song occupies once written as a module of this lineage.

    The header is a fixed slab: the same sample records are written whether a song fills them or leaves
    them empty, so one more sample costs its frames and nothing else. Everything after the header is
    content, which is why the whole size is the header plus what the patterns and the waveforms come to.
    """
    voices = sampled(song)
