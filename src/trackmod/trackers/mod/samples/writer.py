...
def reject_unstorable(sample: Sample) -> None:
    ...
    Raises:
        ValueError: when the sample is stereo, stored at sixteen bits, panned, or carries a sustain loop
            or a loop that plays backwards — each of which this format keeps no field for.
    """
    ...
    return SAMPLE_HEADER.pack(
        {
            ...
            "loop_begin": NO_BEGIN if loop is None else loop.begin // WORD_BYTES,
            "loop_length": NO_LOOP_LENGTH if loop is None else loop.frames // WORD_BYTES,
        }
    )
