...
from trackmod.trackers.mod.spec.effects import (
    NIBBLE_PARAMETER,
    ORDER_PARAMETER,
    PANNING_PARAMETER,
    ROW_PARAMETER,
...
def extended(sub_command: MODExtended, value: int) -> Effect:
    """An ``E`` effect carrying one sub-command and its four-bit value.

    Raises:
        ValueError: when ``value`` exceeds four bits.
    """
    return Effect(command=MODEffect.EXTENDED, parameter=join_nibbles(sub_command, value))


def decimal_parameter(value: int) -> int:
    """The byte this format reads back as the decimal number ``value``.

    A pattern break here reads its parameter a decimal digit to a nibble, which is where every tracker
    descended from this one inherited the reading, so a break to row 16 is stored as ``0x16``.

    Raises:
        ValueError: when ``value`` names a row past the last one a pattern holds.
    """
    return decimal_byte(require_range(value, bound=ROW_PARAMETER, subject="row"))


class MODEffects:
    """The shared effect vocabulary as Amiga ProTracker spells it.

    Speed and tempo share the one ``F`` command, split by where the parameter falls: below the tempo
    floor it sets the ticks a row lasts, and at or above it the beats per minute. This is where
    FastTracker 2 inherited the arrangement, and both spell it with the same command byte.
    """

    def set_speed(self, ticks: int) -> Effect:
        """Set the ticks each row lasts."""
        return Effect(
            command=MODEffect.SET_SPEED,
    ...
    def position_jump(self, order: int) -> Effect:
        ...
        return Effect(
            command=MODEffect.POSITION_JUMP,
    ...
    def pattern_break(self, row: int) -> Effect:
        ...
        return Effect(command=MODEffect.PATTERN_BREAK, parameter=decimal_parameter(row))
